...
const INVOKE_WAIT_TIMEOUT = 15 * 1000;  //调用服务等待超时时间
...
let sessionId = localStorage.getItem("__sessionId__");
...
function handleConnect() {
    ...
    try {
        ...
        websocket.onopen = () => {
            ...
            websocket.send(JSON.stringify({topic: 'system.register', mid: uuid.v1(), sessionId: sessionId, content: {clientInfo: "web", topics: getSubscribeTopics()}}));
        };
        websocket.onmessage = e => {
            ...
            WS_lastMessageTime = new Date().getTime();
            //标记最后通讯时间
            commitState(updateLastMessageTimeAction(WS_lastMessageTime));
        };
        websocket.onerror = e => {
...
export function invokeWebProcessor(apiPath, params, callback) {
    ...
    //
    let msg = {topic: 'system.processor.request', mid, content: {apiPath, params}};
    if(apiPath.indexOf('login') != -1 || apiPath.indexOf('logout') != -1 || apiPath.indexOf('queryUserInfo') != -1) {
