...
export default class BacktestManage extends BaseComponent {
  ...
  constructor(props) {
    super(props);
    //
    this.setState({showTemplateDialog: false, showConfirmDialog1: false, showConfirmDialog2: false, templateInfo: {}, loading: false, currentNode: null, treeNodes:[], expandedKeys:[], properties:[]});
    //
  ...
  startBacktest() {
    ...
    this.invoke("/backtest-manage/startStrategy", {id: this.state.currentNode.key}, (error, data) => {
      if(error) {
        Message.error(error.message);
      } else {
          ...
          this.state.currentNode == null || this.state.currentNode.data.type == 0 ?
            <div style={{flex:1, display:"flex", flexFlow:"column", overflow: "auto"}}>
              <div style={{marginTop:10, marginBottom:10, display:"flex"}}>
                <Button style={{width:'120px'}} type={this.state.loading ? "" : "danger"} disabled={this.state.currentNode == null} onClick={()=>this.startBacktest()}>执行回测</Button> 
              ...
              >
                <Form ref={i => this.templateForm = i} size="default">
                  <FormItem label="模板名称" value={this.state.templateInfo.name} rules={{required: true}}>
                      <Input width={480} value={this.state.templateInfo.name} on$change-value={(c, value) => {this.onFormValueChange("name",value)}}/>
                  </FormItem>
                  <FormItem label="策略服务名" value={this.state.templateInfo.service} rules={{required: true}}>
                      <Input width={480} value={this.state.templateInfo.service} on$change-value={(c, value) => {this.onFormValueChange("service",value)}}/>
                  </FormItem>
                  <FormItem label="市场数据" value={this.state.templateInfo.marketDataSource} rules={{required: true}}>
                      <Input width={480} value={this.state.templateInfo.marketDataSource} on$change-value={(c, value) => {this.onFormValueChange("marketDataSource",value)}} />
                  </FormItem>
                  <FormItem label="交易产品组" value={this.state.templateInfo.productGroup} rules={{required: true}}>
                      <Input width={480} value={this.state.templateInfo.productGroup} on$change-value={(c, value) => {this.onFormValueChange("productGroup",value)}} />
                  </FormItem>
                  <FormItem label="策略规模" value={this.state.templateInfo.size} rules={{required: true}}>
                      <Spinner min={0} width={200} value={this.state.templateInfo.size} on$change-value={(c, value) => {this.onFormValueChange("size",value)}} />
                  </FormItem>
                  <FormItem label="资产类型" value={this.state.templateInfo.currencyType} rules={{required: true}}>
                      <Select width={480} value={this.state.templateInfo.currencyType} on$change-value={(c, value) => {this.onFormValueChange("currencyType",value)}} >
